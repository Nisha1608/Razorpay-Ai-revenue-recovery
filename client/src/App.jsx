...
function analysisSourceLabel(source) {
  return { OPENAI: "OpenAI", GEMINI: "Gemini", DETERMINISTIC_FALLBACK: "Deterministic fallback", DETERMINISTIC: "Deterministic" }[source] || null;
}
...
function App() {
  ...
  return (
    <main className="app-shell">
      <button className={mobileNavOpen ? "mobile-nav-backdrop visible" : "mobile-nav-backdrop"} aria-label="Close navigation" onClick={() => setMobileNavOpen(false)} />
      <aside className={mobileNavOpen ? "sidebar mobile-open" : "sidebar"}>
        ...
        <nav id="primary-navigation" aria-label="Primary navigation">
          {navItems.map(({ label, icon: Icon }) => <button className={page === label ? "nav-item active" : "nav-item"} onClick={() => { setPage(label); setMobileNavOpen(false); }} key={label}><Icon size={17} />{label}</button>)}
        </nav>
...
function CaseDetail({ detail, audit, pending, error, result, onAction, onRetry }) {
  ...
  return <ResourceState resource={detail} onRetry={onRetry}>{() => { const { recoveryCase, customer, payment, action, aiAnalysis, policyEvaluation, execution, journey } = detail.data; return <><section className="detail-intro"><StatusPill status={recoveryCase.status} /><h2>{customer?.name || customer?.email || "Recovery case"}</h2><p>Live recovery state from verified backend records.</p></section>{journey?.attempts?.length ? <section className="info-card"><p className="eyebrow">Recovery journey</p><div className="info-row"><span>Journey status</span><strong>{label(journey.status)}</strong></div>{journey.attempts.map((attempt) => <div className="info-row" key={attempt.attemptNumber}><span>Attempt {attempt.attemptNumber}</span><strong>{label(attempt.recommendedAction)} · {label(attempt.status)}</strong></div>)}</section> : null}<section className="detail-grid"><InfoCard title="Customer" values={[["Name", customer?.name], ["Email", customer?.email], ["Phone", customer?.phone], ["Successful payments", customer?.successfulPayments], ["Failed payments", customer?.failedPayments]]} /><InfoCard title="Payment" values={[["Amount", formatCurrency(payment?.amount)], ["Method", payment?.method], ["Failure", payment?.failureReason], ["Payment status", payment?.status]]} /><InfoCard title="AI analysis" values={[["Risk score", recoveryCase.riskScore], ["Analysis confidence", formatPercent(aiAnalysis?.confidence)], ["Diagnosis", aiAnalysis?.diagnosis || aiAnalysis?.summary], ["Recovery probability", formatPercent(recoveryCase.recoveryProbability)], ["Priority", recoveryCase.priority], ["Recommended action", recoveryCase.recommendedAction], ["Reason", aiAnalysis?.reason || aiAnalysis?.summary]]} /><InfoCard title="Policy decision" values={[["Allowed", policyEvaluation?.allowed === undefined ? "Not evaluated" : policyEvaluation.allowed ? "Allowed" : "Blocked"], ["Reason", policyEvaluation?.reason], ["Evaluated", formatDate(policyEvaluation?.evaluatedAt)]]} /><InfoCard title="Recovery action" values={[["Action type", action?.type], ["Status", action?.status]]} /><InfoCard title="Execution" values={[["Provider reference", execution?.providerReference], ["Payment link", execution?.metadata?.paymentLinkShortUrl], ["Executed", formatDate(execution?.executedAt)], ["Recovery reference", execution?.metadata?.referenceId]]} /></section><MutationPanel pending={pending} error={error} result={result} onAction={onAction} caseId={recoveryCase._id} actionId={action?._id} actionStatus={action?.status} /><Timeline entries={audit.data?.data || []} /></>; }}</ResourceState>;
}
...
function AuditTrail({ audit, selectedCaseId, onRetry }) { if (!selectedCaseId) return <EmptyState message="Select a recovery case from the Recovery Queue to view its audit trail." />; return <ResourceState resource={audit} onRetry={onRetry}>{() => { const entries = audit.data?.data || []; return entries.length ? <section className="table-wrap audit-table"><table><thead><tr><th>Timestamp</th><th>Actor</th><th>Event</th><th>Source</th><th>Action</th><th>Reason</th><th>Policy result</th></tr></thead><tbody>{entries.map((entry) => <tr key={entry._id}><td>{formatDate(entry.createdAt)}</td><td><StatusPill status={entry.actor} /></td><td>{label(entry.eventType)}</td><td>{entry.eventType === "AI_ANALYSIS" ? <span className="analysis-source">{analysisSourceLabel(entry.metadata?.analysisSource) || "Not recorded"}</span> : "--"}</td><td>{label(entry.action?.type)}</td><td>{entry.message}</td><td>{entry.after?.allowed === undefined ? "--" : entry.after.allowed ? "Allowed" : "Blocked"}</td></tr>)}</tbody></table></section> : <EmptyState message="No audit events found for this recovery case." />; }}</ResourceState>; }

function Timeline({ entries }) { return <section className="timeline-panel"><div className="panel-heading"><div><p className="eyebrow">VERIFIED EVENT SEQUENCE</p><h2>Recovery timeline</h2></div></div>{entries.length ? <ol className="timeline-list">{entries.map((entry) => <li key={entry._id}><span className="timeline-dot" /><div><p>{label(entry.eventType)}</p><strong>{entry.message}</strong>{entry.eventType === "AI_ANALYSIS" && analysisSourceLabel(entry.metadata?.analysisSource) ? <span className="analysis-source">{analysisSourceLabel(entry.metadata.analysisSource)}</span> : null}<small>{formatDate(entry.createdAt)} · {entry.actor}</small></div></li>)}</ol> : <EmptyState message="No timeline events found for this recovery case." />}</section>; }
